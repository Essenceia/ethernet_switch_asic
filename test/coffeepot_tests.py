	...
	for i in range(0, TEST_ITER): 
		...
		if not GATES:
			if 2*(i+1) >= table_utils.ENTRY_NUM: 
				assert module.m_switch.m_lookup.m_table.cocotb_nobody_is_dead.value == 1, f"Unexpacted invalid table entry"
			else:
				alloc_cnt =  module.m_switch.m_lookup.m_table.cocotb_entry_alloc_cnt.value
...
async def table_realloc_test_sequence(dut, module ): 
	...
	for i in range(0, TEST_ITER): 
		...
		await ClockCycles(dut.clk, 2*8*4 + 1)
		if not GATES: 
			assert module.m_switch.m_lookup.m_table.cocotb_nobody_is_dead.value == 0, f"Unexpacted multiple allocated table entries"
			assert module.m_switch.m_lookup.m_table.cocotb_entry_alloc_cnt.value == 1, f"Expecting a single allocated table entry"
